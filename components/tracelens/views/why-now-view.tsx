...
export function WhyNowView({ status, onNavigate }: WhyNowViewProps) {
  ...
  const fetchData = async () => {
    try {
      const [wn, wc] = await Promise.all([
        TraceLensAPI.getWhyNow(),
        TraceLensAPI.getWhatChanged()
      ]);
      setWhyNow(wn);
      setWhatChanged(wc);
    } catch (e) {
  ...
  const handleEvaluateRisk = async () => {
    ...
    try {
      const res = await TraceLensAPI.analyzeChange(testService, testVersion, testChangeType, "Pre-deployment risk assessment");
      setChangeRisk(res);
    } catch (e) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="bg-tl-card border border-tl-border rounded-xl p-5 backdrop-blur relative overflow-hidden">
            ...
            <p className="text-xs text-slate-400 mt-1">
              Unlike a simple root cause label, TraceLens identifies the dynamic convergence of multiple environmental factors that precipitated the failure at this specific timestamp.
            </p>
          </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-tl-card border border-tl-border rounded-xl p-5 backdrop-blur space-y-4">
            ...
            <div className="p-3 rounded-lg bg-tl-elevated border border-tl-border text-[11px] text-slate-400 space-y-1">
              ...
              <p>
                TraceLens continuously compares live telemetry against a rolling 30-minute statistical profile. Any metric deviating beyond 3σ is flagged as an active divergence.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-tl-card border border-tl-border rounded-xl p-5 backdrop-blur space-y-4">
        ...
        <p className="text-xs text-slate-400">
          Simulate prospective code, config, or schema changes before pushing to staging or production. TraceLens evaluates downstream topology depth and criticality to estimate blast risk.
        </p>
